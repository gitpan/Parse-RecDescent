...
This document describes version 1.33 of Parse::RecDescent,
...
The token separator for an individual parser (for example, C<$psr>) can be reset by
...
The full set of rules for determining which of several possible
token separators is selected at any point is:

=over 4

=item 1.

If the local variable C<$tokensep> is set, use that.

=item 2.

Otherwise, if the current production's token separator
(that is, C<$thisprod-E<gt>{"tokensep"}>) is set, use that.

=item 3.

Otherwise, if the current rule's token separator
(that is, C<$thisrule-E<gt>{"tokensep"}>) is set, use that.

=item 4.

Otherwise, if the current parsers's token separator
(that is, C<$thisparser-E<gt>{"tokensep"}>) is set, use that.

=item 5.

Otherwise, if the global token separator 
(that is, C<$Parse::RecDescent::tokensep>) is set, use that.

=item 6.

Otherwise, use the default token separator (that is, optional whitespace).

=back

Note that (to reduce overheads) changes in the token separator are only
checked for at the start of a production or after an action.
Furthermore, if the string "tokensep" does not appear anywhere in the
complete grammar, then changes in the token separator are only checked
for at the start of each rule.
...
The current line number within the current parse (starting from 1). For
efficiency, C<$thisline> is actually a tied hash, and only recomputes the
current line number when the variable's value is used.
...
the the use of C<$this...->{local}> is always safe.
...
if it were an unconditional C<E<lt>reject<gt>> directive (that is, it
...
for the C<E<lt>reject<gt>> directive).
...
Productions which start with a C<E<lt>rejectW<gt>> or C<E<lt>rulevar:...<gt>>

...
This document describes version 1.70 of Parse::RecDescent,
...
Regex tokens can be specified using just backslashes as delimiters
...
An action will I<fail> if it's last evaluated value is C<undef>. This is
...
the behaviour of the parser. There are currently forteen directives:
C<E<lt>commitE<gt>>,
C<E<lt>uncommitE<gt>>,
C<E<lt>rejectE<gt>>,
C<E<lt>scoreE<gt>>,
...
is important enough to have it's own subrule, then it's important enough to return).
...
a parser. You simply C<use> the new module, and then call it's

...
This document describes version 1.77 of Parse::RecDescent,
...
whilst all other items are stored under a key that is their ordinal position:

	stuff: /various/ bits 'and' pieces "then" data 'end'
			{ print $item{1}        # PRINTS various
					        # (SIMILAR TO USING @item)

If you want I<named> access to patterns or literals, you need to turn 
...
wil return only the value corresponding to the I<second> match of the
...
Assignment to C<$thiscolumn> or <$prevcolumn> is a fatal error.
...
an earlier C<<score:...E<gt>> directive. $score_return stores
...
those which already end in an action, or those which consist of a
single terminal (i.e. a single pattern or string item).
...
Note that each node in the tree is blessed into a class of the
...
the last two rules produce special objects with the single attribute '__VALUE__'.
This is because they consist solely of a single terminal.
...
C<E<lt>deferE<gt>>,
C<E<lt>nocheck<gt>>,
C<E<lt>perl_quotelike<gt>>,
C<E<lt>perl_codeblock<gt>>,
C<E<lt>perl_variable<gt>>,
...
The C<E<lt>perl_variable<gt>> directive can be used to parse any Perl
...
The C<E<lt>perl_codeblock<gt>> directive can be used to parse curly-brace-delimited block of Perl code, such as: { $a = 1; f() =~ m/pat/; }.
...
The C<<leftop:...E<gt>> and C<<rightop:...E<gt>> directives provide an
easier way of specifying such operations. Using C<<leftop:...E<gt>> the
above examples become:

	disjunction:	<leftop: conjunction 'or' conjunction>
	conjunction:	<leftop: atom 'and' atom>

The C<<leftop:...E<gt>> directive specifies a left-associative binary operator.
...
A C<<leftop:...E<gt>> directive such as:
...
In other words, a C<<leftop:...E<gt>> directive matches the left operand followed by zero
...
(single) value of the entire C<<leftop:...E<gt>> directive.

For example, an C<<leftop:...E<gt>> directive such as:
...
Similarly, the C<<rightop:...E<gt>> directive takes a left operand, an operator, and a right operand:
...
Note that for both the C<<leftop:...E<gt>> and C<<rightop:...E<gt>> directives, the directive does not normally
...
C<<leftop:...E<gt>> and C<<rightop:...E<gt>> directives
...
list, since the C<<leftop:...E<gt>> and C<<rightop:...E<gt>> directives
...
To cater for such situtations, the C<<score:...E<gt>> can be used.
The directive is equivalent to an unconditional C<<rejectE<gt>>,
...
item immediately before its C<<score:...E<gt>> directive is returned as the
result.

In other words, by putting a C<<score:...E<gt>> directive at the end of
...
Now, when each production reaches its respective C<<score:...E<gt>> 
...
Note the use of negation within the C<<score:...E<gt>> directive
...
in a rule use exactly the same C<<score:...E<gt>> directive. It is
tedious to have to repeat this identical directive in every production, so
Parse::RecDescent also provides the C<<autoscore:...E<gt>> directive. 

If an C<<autoscore:...E<gt>> directive appears in any
...
end with an explicit C<<score:...E<gt>> directive. Thus the rules above could
be rewritten:
...
Note that the C<<autoscore:...E<gt>> directive itself acts as an
unconditional C<<rejectE<gt>>, and (like the C<<rulevar:...E<gt>>
...
and produce a class (and module) named PreGrammar, you could use:
...
to be built, and the third is the name of the file in which the class is
to be generated. For example, if the module should be installed as
Grammar::Example::Two you might write:

	Parse::RecDescent->Precompile($grammar,
				      "Grammar::Example::Two",
				      "/lib/perl5/Grammar/Example/Two.pm");
...
a module named Yet::Another::Grammar like so:
...
the same, so whilst precompilation has an effect on E<set-up> speed,
it has no effect on E<parsing> speed. RecDescent 2.0 will address that
problem.
...
Missing or extra components in a C<<leftopE<gt>> or C<<rightopE<gt>>
directive.
...
Productions which consist of a single C<E<lt>error?<gt>>
